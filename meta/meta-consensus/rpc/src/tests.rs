...
async fn get_block_hash() {
    ...
    let meta_consensus_rpc = MetaConsensusRpc::new(client.clone(), None);
...
async fn block_mint() {
    ...
    // mint block using mets-consensus-rpc
    let meta_consensus_rpc = MetaConsensusRpc::new(client.clone(), Some(sink));
