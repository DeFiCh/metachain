...
use meta_runtime::{Hash, Header as MetaHeader};
...
use sc_consensus_manual_seal::{
	...
	ConsensusDataProvider, Error, ManualSealParams,
};
...
use sp_runtime::{
	generic::{BlockId, Digest, DigestItem, SignedBlock},
	traits::Block as BlockT,
};
use std::borrow::BorrowMut;
use substrate_test_runtime::{Block as TestBlock, Extrinsic as TestExtrinsic};
...
async fn get_block_hash() {
	...
	let best_hash_check = || {
		assert_eq!(
			meta_consensus_rpc
				.get_block_hash(client.info().best_number.into())
				.unwrap()
				.to_string(),
			client.info().best_hash.to_string()
		);
	};
...
async fn header_encode_decode() {
	...
	assert_eq!(
		genesis_block.block.header().parent_hash,
		decoded_header.parent_hash
	);
	assert_eq!(
		genesis_block.block.header().number,
		decoded_header.number as u64
	);
	assert_eq!(
		genesis_block.block.header().state_root,
		decoded_header.state_root
	);
	assert_eq!(
		genesis_block.block.header().extrinsics_root,
		decoded_header.extrinsics_root
	);
...
async fn extrinsics_encode_decode() {
	...
	let best_block = client
		.block(&BlockId::Number(client.info().best_number))
		.unwrap()
		.unwrap();
	if best_block.block.extrinsics().len() > 0 {
		...
		assert_eq!(
			best_block.block.extrinsics()[0].transfer().from.to_string(),
			Alice.to_account_id().to_string()
		);
		assert_eq!(
			best_block.block.extrinsics()[0].transfer().from,
			decoded_extrinsic.transfer().from
		);
		assert_eq!(
			best_block.block.extrinsics()[0].transfer().to,
			decoded_extrinsic.transfer().to
		);
		assert_eq!(
			best_block.block.extrinsics()[0].transfer().amount,
			decoded_extrinsic.transfer().amount
		);
...
async fn block_mint() {
	...
	let (dmc_payload, dmc_txs) = meta_consensus_rpc
		.mint_block(Default::default())
		.await
		.unwrap();

	// decode and check results
	assert_eq!(dmc_txs.len(), 0); // for now
	let decoded_signed_block: SignedBlock<TestBlock> =
		SignedBlock::decode(&mut &dmc_payload[..]).unwrap();
	assert_eq!(
		decoded_signed_block.block.header().number as u64,
		block_height_before + 1
	);
	assert_eq!(
		decoded_signed_block.block.header().parent_hash,
		block_hash_before
	);
}
...
async fn block_import_meta_consensus_rpc() {
...

	{
		...
		let import_result = meta_consensus_rpc
			.connect_block(encoded_block, Default::default())
			.await;
		if let Result::Err(jsonrpsee_core::error::Error::Call(CallError::Custom(error_object))) =
			import_result
		{
	...
	}
	{
		...
		let new_block = client
			...
			.unwrap()
			.block;
		let new_signed_block = SignedBlock {
			block: new_block.clone(),
			justifications: None,
		};
		// println!("{:?}", new_signed_block.encode());
		let (import_result, dmc_txs) = meta_consensus_rpc
			.connect_block(new_signed_block.encode(), Default::default())
			.await
			.unwrap();
